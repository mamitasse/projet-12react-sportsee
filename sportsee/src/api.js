...
import { formatUserData, formatUserActivity, formatUserAverageSessions, formatUserPerformance } from './dataFormatter.js';
...
const getUserById = async (id) => {
    if (USE_LOCAL_DATA) {
        ...
        try {
            ...
            userData.score = userData.score !== undefined ? userData.score : userData.todayScore;
    ...
    if (USE_LOCAL_DATA) {
        ...
        try {
            ...
            if (!response.data || !response.data.data || !response.data.data.sessions) {
                throw new Error(`Invalid data structure for user activity with id ${id}`);
            }
...
const getUserAverageSession = async (id) => {
    if (USE_LOCAL_DATA) {
        try {
            const averageSessions = data.USER_AVERAGE_SESSIONS.find(averageSession => averageSession.userId === Number(id));
        ...
        try {
            ...
            if (!response.data || !response.data.data || !response.data.data.sessions) {
                throw new Error(`Invalid data structure for user average sessions with id ${id}`);
            }
...
const getUserPerformance = async (id) => {
    if (USE_LOCAL_DATA) {
        try {
            const performance = data.USER_PERFORMANCE.find(perf => perf.userId === Number(id));
            console.log("Performance data from local:", performance); // Log local data
            if (!performance) throw new Error(`Performances de l'utilisateur avec id ${id} non trouvées`);
        ...
        try {
            ...
            if (!response.data || !response.data.data) {
                throw new Error(`Invalid data structure for user performance with id ${id}`);
            }
